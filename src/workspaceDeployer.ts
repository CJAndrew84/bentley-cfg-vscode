...
import { ProjectWiseClient, PwConnection } from './pwClient';

// ─────────────────────────────────────────────────────────────────────────────
// Public interfaces
// ─────────────────────────────────────────────────────────────────────────────

/** A single file that will be (or was) deployed */
export interface DeploymentFile {
  /** Absolute local path */
  localPath: string;
  /** Path relative to the local workspace root, e.g. "WorkSets/Proj1/WorkSet.cfg" */
  relativePath: string;
  /** Name of the file, e.g. "WorkSet.cfg" */
  fileName: string;
  /** Relative PW folder path, e.g. "WorkSets/Proj1" */
  pwFolderPath: string;
}

/** A pre-flight summary shown to the user before upload starts */
export interface DeploymentPlan {
  localRoot: string;
  targetFolderGuid: string;
  targetFolderLabel: string;
  files: DeploymentFile[];
  /** Unique sub-folder paths that will be created (if absent) */
...
/** Overall result returned after an upload completes */
export interface DeploymentResult {
  created: number;
  updated: number;
  failed: number;
  fileResults: DeploymentFileResult[];
  /** Absolute local path to the generated deploy-csb.ps1 */
  psScriptPath: string;
  /** Absolute local path to the text deployment report */
  reportPath: string;
}
...
function foldersToEnsure(files: DeploymentFile[]): string[] {
  const seen = new Set<string>();
  for (const f of files) {
    if (!f.pwFolderPath) continue;
...
export async function executeDeployment(
  ...
  // ── Step 2: Upload files ───────────────────────────────────────────────────
  const fileResults: DeploymentFileResult[] = [];
 ...
 */
function generatePsScript(plan: DeploymentPlan): string {
...
function detectWorksets(root: string): string[] {
    ...
    if (fs.existsSync(wsDir)) {
      try {
        ...
        for (const e of entries) {
          if (e.isDirectory()) worksets.push(e.name);
        }
      } catch { /* ignore */ }
  ...
  try {
    ...
    for (const e of top) {
      ...
      if (fs.existsSync(wsFile)) worksets.push(e.name);
    }
  } catch { /* ignore */ }
 ...
 */
export function generateDeploymentPackage(plan: DeploymentPlan): string {
