...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('bentley-cfg.importCsbManual', async () => {
        const levelPick = await vscode.window.showQuickPick(csbExtractor_1.CSB_PROCESSING_ORDER.map(l => ({ label: l, description: `%level ${csbExtractor_1.CSB_LEVEL_MAP[l]}` })), { placeHolder: 'Select CSB processing level', title: 'Manual CSB Import' });
        if (!levelPick)
            return;
